...
fn seed_postes(app: &Connection) -> Result<(), String> {
    ...
    for (idx, (rub_set, emp_ids)) in sorted_clusters.iter().enumerate() {
        ...
        // Find the employee with the most payslip records in this cluster
        let representative_emp = {
            let placeholders: Vec<String> = emp_ids.iter().map(|_| "?".to_string()).collect();
            let sql = format!(
                "SELECT employee_id, COUNT(*) as cnt FROM paies WHERE employee_id IN ({}) AND mois != 'TOT-PAIE' GROUP BY employee_id ORDER BY cnt DESC LIMIT 1",
                placeholders.join(",")
            );
            let params: Vec<&dyn rusqlite::ToSql> = emp_ids.iter().map(|id| id as &dyn rusqlite::ToSql).collect();
            app.query_row(&sql, params.as_slice(), |r| r.get::<_, i64>(0))
                .unwrap_or(emp_ids[0])
        };
        ...
        // Use values from montants if available, otherwise 0
        for (sort_idx, rub_code) in rub_set.iter().enumerate() {
            // rub_code is like "R001", convert to "001" for montant lookup
        ...
        // Assign all employees in this cluster to the poste

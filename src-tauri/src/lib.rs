...
fn test_rubrique_formula(
    ...
    let conn = state.conn.lock().map_err(|e| e.to_string())?;
    let _emp_id = employee_id.unwrap_or(1);
...
async fn get_all_salary_history(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let (sql2, params2): (String, Vec<Box<dyn rusqlite::ToSql>>) = if let Some(ref p) = period {
            let patterns = period_to_paies_patterns(p);
            // Also include patterns with S and O suffixes
            let mut all_patterns: Vec<String> = Vec::new();
            for pat in &patterns {
                all_patterns.push(pat.clone());
                all_patterns.push(format!("{}S", pat));
                all_patterns.push(format!("{}O", pat));
            }
            // Build placeholders
            let placeholders: Vec<String> = all_patterns.iter().enumerate().map(|(i, _)| format!("?{}", i + 1)).collect();
            ...
            let sql = format!(
                r#"SELECT p.id, p.employee_id, p.mois, p.matricule, p.montants, p.sit_fam, p.nbre_enf,
                        ...
                        WHERE p.mois != 'TOT-PAIE' AND p.mois IN ({})
            ...
            (r#"SELECT p.id, p.employee_id, p.mois, p.matricule, p.montants, p.sit_fam, p.nbre_enf,
                        ...
                        ORDER BY p.mois DESC, e.nom, e.prenom LIMIT 500"#.to_string(),
             Vec::new())
        };
        let mut stmt2 = conn.prepare(&sql2).map_err(|e| e.to_string())?;
        let param_refs: Vec<&dyn rusqlite::ToSql> = params2.iter().map(|p| p.as_ref()).collect();
        let rows2 = stmt2.query_map(param_refs.as_slice(), |row| {
...
fn normalize_period(p: &str) -> Option<String> {
    // Accepts: YYYY-MM, MM-AAAA, MM/AAAA, MMYYYY, with optional trailing letters (S, O)
    let p = p.trim();
    // Strip trailing non-numeric characters (e.g. "12-2023S" -> "12-2023")
    let p: String = p.chars().rev().skip_while(|c| !c.is_ascii_digit()).collect::<String>().chars().rev().collect();
...
fn get_employee_salary_history(
    ...
    let sql = if only_real {
        "SELECT mois, montants, c_date FROM paies WHERE employee_id=? AND mois GLOB '[0-9][0-9]-[0-9][0-9][0-9][0-9]' ORDER BY mois"
    } else {
        "SELECT mois, montants, c_date FROM paies WHERE employee_id=? ORDER BY mois"
    };
    let mut stmt = conn.prepare(sql).map_err(|e| e.to_string())?;
    ...
    for row in rows {
        let (mois, montants, _c_date) = row.map_err(|e| e.to_string())?;
